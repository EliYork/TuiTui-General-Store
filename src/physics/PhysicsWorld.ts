...
export class PhysicsWorld {
  private readonly landingResolver: LandingResolver;

  constructor(private readonly config: PhysicsConfig) {
    this.landingResolver = new LandingResolver(config);
  }

  updateItems(items: BoardItem[], pusher: Pusher, deltaSeconds: number): PhysicsStepResult {
  ...
  private integrateItems(items: BoardItem[], deltaSeconds: number): void {
    for (const item of items) {
      if (item.isDropped) {
        continue;
      }

      item.applyLinearDamping(
        this.config.linearDamping,
        deltaSeconds,
        this.config.restThreshold
      );
  ...
  private resolvePusherPush(items: BoardItem[], pusher: Pusher): void {
    ...
    for (const item of items) {
      ...
      const overlapsY =
        item.depth + item.radius >= bounds.y &&
  ...
  private resolveItemPairs(items: BoardItem[]): void {
    for (let index = 0; index < items.length; index += 1) {
      ...
      for (let otherIndex = index + 1; otherIndex < items.length; otherIndex += 1) {
        ...
        if (
          itemA.height > this.config.airborneCollisionHeight ||
  ...
  private resolveTableBounds(items: BoardItem[]): void {
    for (const item of items) {
      if (item.isDropped) {
        continue;
      }
  ...
  private collectDroppedItems(items: BoardItem[], droppedItems: DroppedItemResult[]): void {
    for (const item of items) {
      if (item.isDropped) {
        continue;
      }
  ...
  private integrateHeight(item: BoardItem, deltaSeconds: number): void {
    ...
    item.heightVelocity = Math.max(
      -this.config.maxHeightVelocity,
      item.heightVelocity - this.config.heightGravity * deltaSeconds
    );
  ...
  private relaxSupportStates(items: BoardItem[], deltaSeconds: number): void {
    for (const item of items) {
