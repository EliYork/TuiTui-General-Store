...
### 新增经营日记 MVP，并修复经营结算误触问题

- 修改内容：
  - 新增局内自动经营日记 MVP，每次开始经营模式自动创建新日记。
  - 日记开头记录：日记格式版本、游戏版本、构建类型、运行平台、本局编号、开始时间、场景、初始配置。
  - 新增版本常量文件 `assets/scripts/config/ProjectVersion.ts`，包含 `PROJECT_VERSION` 和 `DIARY_FORMAT_VERSION`。
  - 日记记录事件：第 X 天开始、第 X 天结束经营、商店购买、进入下一天、经营失败（如已实现）。
  - 主菜单增加经营日记入口，支持查看、复制、清空。
  - 日记文本移除"投放资源：X / Y"无效字段，只保留"剩余进货：X"。
  - 修复未达标时点击"结束经营"也会写入结束经营日记的问题。
  - 修复未达标时可能发放经营加成奖励、改变资金的问题。
  - 增加同一天防重复结算锁，避免多次点击结束经营导致重复写日记或重复发奖励。
  - 达标后正常结算仍写入"第 X 天结束经营"记录；未达标时点击结束经营只提示，不结算、不写结束经营日记、不发奖励、不改变资金。
- 涉及文件：
  - `assets/scripts/config/ProjectVersion.ts`（新增）
  - 经营日记相关脚本（DiaryManager 或同类）
  - `BusinessModeController.ts`（结算防重复锁、日记写入、资金保护）
  - 主菜单场景（日记入口按钮）
- 手动验收：
  - 日记开头能显示本局信息和初始配置。
  - 日记中不再出现"投放资源：100 / 0"。
  ...
  - 商店购买正常写入日记。
  - 进入下一天正常写入日记。
  - 未达标重复点击结束经营不再写入多条未达标结束记录。
- 后续注意：
  - Auto 按钮与自动结算是否只触发一次，需继续回归。
  - 停滞检测 / 经营失败记录是否与日记系统正确配合。
