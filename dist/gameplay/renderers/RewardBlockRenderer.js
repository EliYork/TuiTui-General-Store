...
exports.RewardBlockRenderer = void 0;
class RewardBlockRenderer {
    render(context, rewardBlock, config, projector) {
        ...
        const bodyBaseCenter = projector.projectToScreen(rewardBlock.x, rewardBlock.depth, rewardBlock.totalVisualHeight, 0);
        const topCenter = projector.projectToScreen(rewardBlock.x, rewardBlock.depth, rewardBlock.totalVisualHeight, rewardBlock.stackLevel);
        ...
        context.beginPath();
        context.ellipse(shadowCenter.x + shadowOffset.x, shadowCenter.y + shadowOffset.y, halfWidth * 0.95, topHeight * config.pseudo3d.shadowScaleY, 0, 0, Math.PI * 2);
        context.fill();
        if (rewardBlock.rewardType === "gemReward") {
            this.drawGem(context, rewardBlock, colors, topCenter, bodyBaseCenter, halfWidth, topHeight);
        }
        else {
            this.drawBlock(context, rewardBlock, colors, topCenter, bodyBaseCenter, halfWidth, topHeight, thickness);
        }
        context.restore();
    }
    drawBlock(context, rewardBlock, colors, topCenter, bodyBaseCenter, halfWidth, topHeight, thickness) {
    ...
    drawGem(context, rewardBlock, colors, topCenter, bodyBaseCenter, halfWidth, topHeight) {
        context.beginPath();
    ...
    getRewardColors(rewardType, config) {
...
exports.RewardBlockRenderer = RewardBlockRenderer;
